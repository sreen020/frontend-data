...
export function showSellingpoint(data) {
  ...
  function filterPointsByYears(data) {
    ...
    data.forEach((element) => {
      arrayWithAllYears.push(
        element.startdatesellingpoint
          ? element.startdatesellingpoint.slice(0, 4)
          : null
      );
    });
    ...
    function changeYear(i) {
      const filteredYear = data.filter((row) =>
        row.startdatesellingpoint
          ? row.startdatesellingpoint.slice(0, 4) == i
          : null
      );
